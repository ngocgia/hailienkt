...
$sum = $db->count_list_dich_vu($active,$id_type,$search);
$limit = 20;
$pages = (($sum%$limit)==0)?$sum/$limit:floor($sum/$limit)+1;
$start = ($num-1)*$limit;
$array = $db->get_list_dich_vu($active,$id_type,$search,$orderby,$start,$limit);

$rel = '';
if(strlen($id_type)>0){$rel .= '&id_type='.$id_type;}

require_once("views/dichvu/list.php");
unset($db);
