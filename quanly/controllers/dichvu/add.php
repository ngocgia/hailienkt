...
if(isset($_POST['add-bt'])==true){	
	...
	if ($flag==1){
		if (strlen($image)>0){			
			...
			if($tam==1){
				...
				$resize->saveImage('../images/dichvu/'.$image, "50");
			}else{
    ...
    if($flag){
        ...
        $db->create_row_dich_vu($name,$id_type,$image,$image_slide,$short_info,$info,$key_search,$dayup,$daycrea,$race,$active);
        $error = 'Tạo Thành Công!';
    }
}

require_once("views/dichvu/add.php");
unset($db);
